...
class DataManager {
  ...
  /** Write an arbitrary JS object into a .md file.
   *
   *  File structure:
   *    1. YAML frontmatter  — Dataview-queryable properties (no base64 blobs)
   *    2. EM_DATA block     — Full JSON payload for plugin reads (unchanged)
   *    3. Human-readable MD — Rendered by renderFn for normal Obsidian viewing
   *
   *  The `type` hint drives which frontmatter fields are emitted.
   *  Recognised types: 'series' | 'movie' | 'book' | 'tvlog' | 'movielog' | 'readlog'
   */
  async writeMD(path, data, renderFn, type) {
    await this.ensureDir(path.substring(0, path.lastIndexOf('/')));
    const frontmatter = this._buildFrontmatter(data, type);
    const dataBlock   = `${EM_DATA_START}\n${JSON.stringify(data, null, 2)}\n${EM_DATA_END}`;
    const readable    = renderFn ? renderFn(data) : '';
    const content     = `${frontmatter}${dataBlock}\n\n${readable}`;
    const f = this.app.vault.getAbstractFileByPath(path);
    if (f) await this.app.vault.modify(f, content);
    else   await this.app.vault.create(path, content);
  }

  /** Build a YAML frontmatter block from a data object.
   *
   *  Rules:
   *   - base64 image fields (logo, cover, image) are always excluded
   *     because they are huge and Dataview cannot use them.
   *   - Arrays (seasons, episodes, log entries) are serialised as
   *     YAML lists of simple scalars where possible; complex objects
   *     are skipped to keep frontmatter readable.
   *   - All values are stringified safely (no unquoted colons / special chars).
   */
  _buildFrontmatter(data, type) {
    if (!data || typeof data !== 'object') return '';

    // Fields that are always excluded from frontmatter
    const SKIP = new Set(['logo', 'cover', 'image', 'poster']);

    const yamlVal = (v) => {
      if (v === null || v === undefined || v === '') return '""';
      if (typeof v === 'number') return String(v);
      if (typeof v === 'boolean') return String(v);
      // Strings: wrap in quotes if they contain special YAML characters
      const s = String(v);
    ...
    // Helper: emit a scalar field
    const field = (key, val) => {
      if (val === undefined || val === null) return;
      lines.push(`${key}: ${yamlVal(val)}`);
    };

    // Helper: emit a flat string array field
    const listField = (key, arr) => {
      if (!Array.isArray(arr) || arr.length === 0) return;
      lines.push(`${key}:`);
      arr.forEach(v => lines.push(`  - ${yamlVal(v)}`));
    };

    // ---- Type-specific field selection ----
    if (type === 'series') {
      field('em_type', 'tv-series');
      field('em_id', data.id);
      field('title', data.name);
      field('status', data.status);
      field('genre', data.genre);
      field('created_by', data.createdBy);
      field('network', data.network);
      field('where_to_watch', data.whereToWatch);
      field('content_rating', data.contentRating);
      field('premiere_date', data.premiereDate);
      field('finale_date', data.finaleDate);
      field('total_seasons', data.totalSeasons);
      field('total_episodes', data.totalEpisodes);
      const seasonNums = (data.seasons || []).map(s => `Season ${s.number}`);
      listField('seasons', seasonNums);
      ...
      field('genre', data.genre);
      field('release_year', data.releaseYear);
      field('runtime_min', data.runtime);
      ...
      field('published_year', data.publishedYear);
      field('rating', data.rating);
      field('status', data.status);

    } else if (type === 'tvlog') {
      // Array of log entries — emit summary stats + entry count
      const entries = Array.isArray(data) ? data : [];
      field('em_type', 'tv-watchlog');
      field('entry_count', entries.length);
      if (entries.length) {
        field('last_watched', entries[entries.length - 1].date || '');
        const avgRating = entries.reduce((s, e) => s + (e.rating || 0), 0) / entries.length;
        field('avg_rating', Math.round(avgRating * 10) / 10);
        const seriesSet = [...new Set(entries.map(e => e.seriesName).filter(Boolean))];
        listField('series_tracked', seriesSet);
      }

    } else if (type === 'movielog') {
      const entries = Array.isArray(data) ? data : [];
      field('em_type', 'movie-watchlog');
      field('entry_count', entries.length);
      if (entries.length) {
        field('last_watched', entries[entries.length - 1].date || '');
        const avgRating = entries.reduce((s, e) => s + (e.rating || 0), 0) / entries.length;
        field('avg_rating', Math.round(avgRating * 10) / 10);
        const movieSet = [...new Set(entries.map(e => e.movieTitle).filter(Boolean))];
        listField('movies_tracked', movieSet);
      }

    } else if (type === 'readlog') {
      const entries = Array.isArray(data) ? data : [];
      field('em_type', 'reading-log');
      field('entry_count', entries.length);
      if (entries.length) {
        const totalPages = entries.reduce((s, e) => s + (Number(e.pagesRead) || 0), 0);
        field('total_pages_read', totalPages);
        const avgRating = entries.reduce((s, e) => s + (e.rating || 0), 0) / entries.length;
        field('avg_rating', Math.round(avgRating * 10) / 10);
        const bookSet = [...new Set(entries.map(e => e.bookTitle).filter(Boolean))];
        listField('books_tracked', bookSet);
      }

    } else {
      // Generic fallback: emit all scalar fields, skip blobs and objects
      field('em_type', 'unknown');
      Object.entries(data).forEach(([k, v]) => {
        if (SKIP.has(k)) return;
        if (typeof v === 'object') return; // skip nested objects/arrays in generic mode
        field(k, v);
      });
    }
  ...
  async saveSeries(series) {
    ...
    await this.writeMD(this.seriesPath(series.id), series, d => this._renderSeries(d), 'series');
    return series;
  }
  ...
  async addMovie(movie) {
    ...
    await this.writeMD(this.moviePath(movie.id), movie, d => this._renderMovie(d), 'movie');
    return movie;
  }

  async updateMovie(updated) {
    await this.writeMD(this.moviePath(updated.id), updated, d => this._renderMovie(d), 'movie');
  }
  ...
  async addBook(book) {
    ...
    await this.writeMD(this.bookPath(book.id), book, d => this._renderBook(d), 'book');
    return book;
  }

  async updateBook(updated) {
    await this.writeMD(this.bookPath(updated.id), updated, d => this._renderBook(d), 'book');
  }
  ...
  async saveTVWatchlog(logs) {
    await this.writeMD(this.watchlogTVFile, logs, d => this._renderTVWatchlog(d), 'tvlog');
  }
  ...
  async saveMovieWatchlog(logs) {
    await this.writeMD(this.watchlogMovieFile, logs, d => this._renderMovieWatchlog(d), 'movielog');
  }
  ...
  async saveReadlog(logs) {
    await this.writeMD(this.readlogFile, logs, d => this._renderReadlog(d), 'readlog');
  }
