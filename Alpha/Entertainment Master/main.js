...
class DataManager {
  constructor(app) {
    ...
    this.movieFile = 'Entertainment/Movies/movies.json';
    this.bookFile = 'Entertainment/Books/books.json';
    this.watchlogTVFile = 'Entertainment/TV/watchlog.json';
    this.watchlogMovieFile = 'Entertainment/Movies/watchlog.json';
    this.readlogFile = 'Entertainment/Books/readlog.json';
  }
  ...
  async writeJSON(path, data) {
    ...
    if (f) await this.app.vault.modify(f, txt);
    else await this.app.vault.create(path, txt);
  }

  // ---- TV Series ----
  seriesPath(id) { return `${this.tvDir}/${id}.json`; }

  async getAllSeries() {
    const folder = this.app.vault.getAbstractFileByPath(this.tvDir);
    if (!folder) return [];
    const files = folder.children ? folder.children.filter(f => f.extension === 'json' && f.name !== 'watchlog.json') : [];
    const all = [];
  ...
  async getSeries(id) {
    return this.readJSON(this.seriesPath(id), null);
  }

  async saveSeries(series) {
    if (!series.id) series.id = generateId();
    await this.writeJSON(this.seriesPath(series.id), series);
    return series;
  }
  ...
  // ---- Movies ----
  async getAllMovies() { return this.readJSON(this.movieFile, []); }
  async saveMovies(movies) { await this.writeJSON(this.movieFile, movies); }
  ...
  async updateMovie(updated) {
    ...
    if (idx !== -1) { movies[idx] = updated; await this.saveMovies(movies); }
  }
  async deleteMovie(id) {
    const movies = (await this.getAllMovies()).filter(m => m.id !== id);
    await this.saveMovies(movies);
  }

  // ---- Books ----
  async getAllBooks() { return this.readJSON(this.bookFile, []); }
  async saveBooks(books) { await this.writeJSON(this.bookFile, books); }
  async addBook(book) {
  ...
  async updateBook(updated) {
    ...
    if (idx !== -1) { books[idx] = updated; await this.saveBooks(books); }
  }
  async deleteBook(id) {
    const books = (await this.getAllBooks()).filter(b => b.id !== id);
    await this.saveBooks(books);
  }

  // ---- TV Watchlog ----
  async getTVWatchlog() { return this.readJSON(this.watchlogTVFile, []); }
  async saveTVWatchlog(logs) { await this.writeJSON(this.watchlogTVFile, logs); }
  ...
  // ---- Movie Watchlog ----
  async getMovieWatchlog() { return this.readJSON(this.watchlogMovieFile, []); }
  async saveMovieWatchlog(logs) { await this.writeJSON(this.watchlogMovieFile, logs); }
  ...
  // ---- Reading Log ----
  async getReadlog() { return this.readJSON(this.readlogFile, []); }
  async saveReadlog(logs) { await this.writeJSON(this.readlogFile, logs); }
